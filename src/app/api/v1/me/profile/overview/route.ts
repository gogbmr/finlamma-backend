...
registry.registerPath({
  ...
  summary: "Get my profile overview (Profile screen ID card, PR-01/PR-02)",
  description:
    "Kid-safe identity (first name + last initial only - never a full name or photo, " +
    "CLAUDE.md rule 10), joined date, level, XP progress to the next level, and the rank " +
    "title the caller's current level currently qualifies for (admin-editable rank_titles " +
    "table, or null if none applies yet). Percentile rank is omitted until Phase 6 ships " +
    "Arena's weekly leaderboard snapshot (docs/FEATURE_MAP.md PR-03) - before that, only " +
    "self-progress is shown.",
  tags: ["Learning"],
  security: [{ bearerAuth: [] }],
  responses: {
    200: {
